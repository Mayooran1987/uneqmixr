...
##' n <- 10
##' n_sim <- 100000
##' scenario_2_OC(c, mulow, muhigh, sd, m1, m2, n, n_sim)
##' @usage  scenario_2_OC(c, mulow, muhigh, sd, m1, m2, n, n_sim)
##' @export
scenario_2_OC <- function(c, mulow, muhigh, sd = 0.8, m1, m2, n, n_sim){
  P_a <- NULL
  ...
  for (i in 1:length(mu)) {
    # for (j in 1:2) {
      ...
      Pa[i,1] <-  scenario_2_pa(c, mu[i], sd, m1, n, n_sim)
      Pa[i,2] <-  scenario_2_pa(c, mu[i], sd, m2, n, n_sim)
    # }
  }
  # pa <- (1-pd)^n
