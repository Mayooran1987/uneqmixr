...
##' @title Probability of acceptance  estimation when lot with homogeneous contaminations based on simulations.
...
##' @details \code{\link{scenario_1B_pa}} provides a probability of acceptance  under scenario 1 of modelling the quantity of material sampled in the risk assessment study.
...
##' @return Probability of acceptance  when lot with homogeneous contaminants by simulations results.
##' @examples
##' c <- 0
##' mu <- -3
##' sd <- 0.8
...
##' @usage  scenario_1B_pa(c, mu, sd, m, n, n_sim)
##' @export
scenario_1B_pa <- function(c, mu, sd = 0.8, m, n, n_sim){
  pd <- scenario_1B_pd(mu, sd, m, n_sim)
