##' \code{\link{scenario_4_pd_curve}} provides the probability of detection curves under scenario 4 of modelling the quantity of material sampled in the risk assessment study.
##' @title Construction of probability of detection curves under lot with homogeneous contaminations and concentration levels fluctuating from sublot to sublot based on simulations results.
...
##' @details \code{\link{scenario_4_pd_curve}} provides the probability of detection curves under scenario 4 of modelling the quantity of material sampled in the risk assessment study. (this section will be updated later on)
...
##' sd_b <- 0.2
##' sd_w <- 0.8
##' scenario_4_pd_curve(mulow, muhigh, sd_b, sd_w = 0.8, m11, m22, type = "theory")
##' scenario_4_pd_curve(mulow, muhigh, sd_b, sd_w = 0.8, m11, m22, type = "simulation", n_sim = 1000000)
##' @usage  scenario_4_pd_curve(mulow, muhigh, sd_b, sd_w = 0.8, m11, m22, type, n_sim)
##' @export
scenario_4_pd_curve <- function(mulow, muhigh, sd_b, sd_w = 0.8, m11, m22, type, n_sim = NA) {
  p_d <- NULL
  ...
  if (type == "theory") {
    ...
    for (i in 1:length(mu)) {
      # for (j in 1:2) {
      Pd[i, 1] <- scenario_4_pd(mu[i], sd_b, sd_w = 0.8, m11, type = "theory")
      Pd[i, 2] <- scenario_4_pd(mu[i], sd_b, sd_w = 0.8, m22, type = "theory")
      # }
    }
    ...
    if (is.na(n_sim) == TRUE) {
      ...
      for (i in 1:length(mu)) {
        # for (j in 1:2) {
        Pd[i, 1] <- scenario_4_pd(mu[i], sd_b, sd_w = 0.8, m11, type = "simulation", n_sim)
        Pd[i, 2] <- scenario_4_pd(mu[i], sd_b, sd_w = 0.8, m22, type = "simulation", n_sim)
        # }
      }
    }
  } else {
