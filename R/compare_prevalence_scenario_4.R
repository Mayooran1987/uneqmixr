##' \code{\link{compare_prevalence_scenario_4}} provides graphical displays based on prevalence before inspection under scenario 4 of modelling the quantity of material sampled in the risk assessment study.
##' @title Graphical displays of prevalence before inspection of different sampling schemes under scenario 4.
...
##' @details \code{\link{compare_prevalence_scenario_4}} provides graphical displays based on prevalence before inspection under scenario 4 of modelling the quantity of material sampled in the risk assessment study. (this section will be updated later on)
##' @return Graphical displays based on prevalence before inspection when lot with homogeneous contamination and contamination levels fluctuate from lot to lot by using theoretical or simulation-based results.
##' @examples
##' mulow <- -6
##' muhigh <- 1
...
##' 10,5,10,15,5)
##' l <- 5000
##' compare_prevalence_scenario_4(mulow, muhigh, sd = 0.8, m1, m2, l, type = "theory")
##' @usage  compare_prevalence_scenario_4(mulow, muhigh, sd, m1, m2, l, type, n_sim)
##' @export
compare_prevalence_scenario_4 <- function(mulow, muhigh, sd = 0.8, m1, m2, l, type, n_sim = NA) {
  ...
  #   # for (j in 1:2) {
  #   Prev[i,1] <-  scenario_4_prevalence(mu[i], sd = 0.8, m1, l, type, n_sim)
  #   Prev[i,2] <-  scenario_4_prevalence(mu[i], sd = 0.8, m2, l, type, n_sim)
  #   # }
  # }
  if (type == "theory") {
    Prev <- matrix(NA, nrow = length(mu), ncol = 2)
    for (i in 1:length(mu)) {
      # for (j in 1:2) {
      Prev[i, 1] <- scenario_4_prevalence(mu[i], sd = 0.8, m1, l, type = "theory")
      Prev[i, 2] <- scenario_4_prevalence(mu[i], sd = 0.8, m2, l, type = "theory")
      # }
    }
    Prob <- data.frame(mu, Prev)
    colnames(Prob) <- c("mu", f_spr(m1), f_spr(m2))
    melten.Prob <- reshape2::melt(Prob, id = "mu", variable.name = "Sampling_scheme", value.name = "prevalence")
    plot_sam <- ggplot2::ggplot(melten.Prob, ggplot2::aes(x = mu, y = prevalence, group = Sampling_scheme, colour = Sampling_scheme)) +
      ggplot2::geom_line() +
      # ggplot2::geom_smooth() +
      # ggplot2::ggtitle("OC curve based on Lognormal distribution") +
      ggplot2::theme_classic() +
      ggplot2::xlab(expression("log mean concentration  (" ~ mu * ~")")) +
      ggplot2::ylab(expression(Prevalance)) +
      ggthemes::scale_colour_colorblind() +
      ggplot2::theme(
        plot.title = ggplot2::element_text(hjust = 0.5), legend.position = c(0.25, 0.75), axis.line.x.top = ggplot2::element_line(color = "red"),
    ...
    if (is.na(n_sim) == TRUE) {
      ...
      for (i in 1:length(mu)) {
        # for (j in 1:2) {
        Prev[i, 1] <- scenario_4_prevalence(mu[i], sd = 0.8, m1, l, type = "simulation", n_sim)
        Prev[i, 2] <- scenario_4_prevalence(mu[i], sd = 0.8, m2, l, type = "simulation", n_sim)
        # }
      }
