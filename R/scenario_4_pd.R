##' \code{\link{scenario_4_pd}} provides a probability of detection under scenario 4 of modelling the quantity of material sampled in the risk assessment study.
##' @title Probability of detection estimation when lot with homogeneous contaminations and concentration levels fluctuating from sublot to sublot.
...
##' @details \code{\link{scenario_4_pd}} provides a probability of detection under scenario 4 of modelling the quantity of material sampled in the risk assessment study.
...
##' sd_w <- 0.8
##' m <- list(m1,m2,m3,m4)
##' scenario_4_pd(mu, sd_b, sd_w = 0.8, m, type = "theory")
##' scenario_4_pd(mu, sd_b, sd_w = 0.8, m, type = "simulation", n_sim = 20000)
##' @usage  scenario_4_pd(mu, sd_b, sd_w, m, type, n_sim)
##' @export
scenario_4_pd <- function(mu, sd_b, sd_w = 0.8, m, type, n_sim = NA) {
  set.seed(1000)
  mu1 <- rnorm(length(m), mu, sd_b)
  sim2 <- matrix(NA, nrow = 1, ncol = length(m))
  for (j in 1:length(m)) {
    sim2[, j] <- 1 - scenario_1_pd(mu1[j], sd_w, unlist(m[j]), type, n_sim)
  }
  p_d <- 1 - prod(sim2)
  return(p_d)
}
